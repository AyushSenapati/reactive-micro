...
func (ep *ePolicy) upsert(fp Policy) error {
	...
	if err != nil {
		msg := fmt.Sprint("merging fep error. err:", err)
		fmt.Println(msg)
...
func getPolicyFromString(s string) (Policy, error) {
	...
	c := strings.Split(s, ":")
	if len(c) != 4 {
		return p, errors.New("invalid policy")
	}
...
func (cps *cachedPolicyStorage) UpdatePolicy(method, sub, rtype, rid, act string) error {
	...
	if !found {
		msg := fmt.Sprintf("authz cache: rtype-%s not supported by this svc. skip re-caching", rtype)
		return errors.New(msg)
	}
	...
	if !found {
		msg := fmt.Sprintf("authz cache: policy for sub-%s not found. skip fetching", sub)
		return errors.New(msg)
	}
	...
	fmt.Printf("authz cache: updated[%s] cache for sub-%s\n", method, sub)
...
func NewPolicyEnforcer(ps PolicyStorage) (PolicyEnforcer, error) {
	if ps == nil {
		return nil, errors.New("policy storage can not be nil")
	}
